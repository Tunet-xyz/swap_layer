...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not result['valid']:
            ...
            self.stdout.write('')
            self.stdout.write(self.style.WARNING('Hint: Make sure SWAPLAYER is configured in your Django settings.py'))
            self.stdout.write('')
        ...
        for field_name, field_value in config.__dict__.items():
            ...
            if any(keyword in field_name.lower() for keyword in ['key', 'secret', 'token', 'password']):
                if isinstance(field_value, str):
                    masked = field_value[:8] + '...' + field_value[-4:] if len(field_value) > 12 else '***'
