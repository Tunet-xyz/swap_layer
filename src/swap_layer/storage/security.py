...
class StorageSecurityContext:
    ...
    permissions: set[StoragePermission] | None = None
    ...
    def system_context(cls, reason: str = "system operation") -> "StorageSecurityContext":
        ...
        """
        logger.warning(f"Creating system-level StorageSecurityContext - reason: {reason}")
        return cls(
            scope_format="",
            user_id="system",
            role="system",
...
def validate_storage_context(
    ...
    required_permissions: set[StoragePermission] | None = None,
    ...
    if not context.is_system:
        if not context.organization_id:
            raise ValueError("Storage context must have organization_id for tenant isolation")

    if require_user and not context.user_id:
        raise ValueError("Storage operation requires an authenticated user_id in the context")
...
class ScopedStorageProvider:
    ...
    def generate_presigned_upload_url(
        ...
        return self._provider.generate_presigned_upload_url(scoped_path, content_type, expiration)
...
def storage_context(
    provider,
    context: StorageSecurityContext | None = None,
):
    """
    ...
    try:
        ...
        logger.error(
            f"Storage operation error: {e} (user={context.user_id}, org={context.organization_id})"
        )
        raise
    finally:
        logger.debug(
