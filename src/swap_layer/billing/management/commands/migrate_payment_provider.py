...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'from_provider',
            type=str,
            help='Current provider (e.g., "stripe")'
        )
        parser.add_argument(
            'to_provider',
            type=str,
            help='Target provider (e.g., "paypal")'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        customers = Model.objects.filter(
            payment_provider=from_provider
        ).exclude(
        ...
        if count == 0:
            self.stdout.write(self.style.WARNING(
                f'No customers found using provider "{from_provider}"'
            ))
            return
            ...
            if dry_run:
                ...
                try:
                    ...
                    self.stdout.write(self.style.ERROR(
                        f'  ✗ Failed: {customer_identifier} - {str(e)}'
                    ))
                    failed += 1
