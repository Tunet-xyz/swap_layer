...
class PayPalPaymentProvider(PaymentProviderAdapter):
    ...
    def _money(amount: Decimal | int | str, currency: str) -> dict[str, str]:
        value = (
            str(amount) if isinstance(amount, str) else str(Decimal(str(amount)) / Decimal("100"))
        )
    ...
    def create_product(
        ...
        **extra_params: Any,
        ...
        product = self._request(
            "POST", "/v1/catalogs/products", json=payload, idempotency_key=idempotency_key
        )
    ...
    def create_price(
        ...
        **extra_params: Any,
        ...
        payload.update(extra_params)
        plan = self._request(
            "POST", "/v1/billing/plans", json=payload, idempotency_key=idempotency_key
        )
    ...
    def create_subscription(
        ...
        **extra_params: Any,
        ...
        subscription = self._request(
            "POST", "/v1/billing/subscriptions", json=payload, idempotency_key=idempotency_key
        )
        return self._normalize_subscription(subscription)

    def get_subscription(self, subscription_id: str) -> dict[str, Any]:
        return self._normalize_subscription(
            self._request("GET", f"/v1/billing/subscriptions/{subscription_id}")
        )
            ...
            patches.append(
                {
                    "op": "replace",
                    "path": "/custom_id",
                    "value": metadata.get("custom_id") or metadata.get("id"),
                }
            )
        if patches:
    ...
    def create_payment_intent(
        ...
        **extra_params: Any,
        ...
        if metadata:
            payload["purchase_units"][0]["custom_id"] = metadata.get("custom_id") or metadata.get(
                "id"
            )
        payload.update(extra_params)
        order = self._request(
    ...
    def get_payment_intent(self, payment_intent_id: str) -> dict[str, Any]:
        return self._normalize_order(
            self._request("GET", f"/v2/checkout/orders/{payment_intent_id}")
        )
    ...
    def create_invoice(self, customer_id: str, **kwargs: Any) -> dict[str, Any]:
        payload = {
            ...
            "primary_recipients": kwargs.pop("primary_recipients", []),
        }
    ...
    def void_invoice(self, invoice_id: str, **kwargs: Any) -> dict[str, Any]:
        self._request(
            ...
            json=kwargs or {"subject": "Invoice voided"},
        )
    ...
    def create_refund(
        ...
        **kwargs: Any,
        ...
        if not capture_id:
            raise PaymentValidationError(
                "PayPal refunds require a capture_id via charge_id or payment_intent_id"
            )
        payload = {}
    ...
    def _normalize_price(self, plan: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "amount": self._amount_from_money(price),
            "currency": price.get("currency_code"),
            "recurring": cycle.get("frequency"),
    ...
    def _normalize_subscription(self, subscription: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "customer_id": (
                (subscription.get("subscriber") or {}).get("payer_id")
                or (subscription.get("subscriber") or {}).get("email_address")
            ),
            ...
            "canceled_at": None,
            "items": [{"price_id": subscription.get("plan_id"), "quantity": 1}],
            "metadata": {"custom_id": subscription.get("custom_id")}
            if subscription.get("custom_id")
            else {},
            "url": self._find_approval_url(subscription),
        }
    ...
    def _normalize_order(self, order: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "client_secret": None,
            "metadata": {},
            "url": self._find_approval_url(order),
        }
    ...
    def _normalize_invoice(self, invoice: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "id": invoice.get("id"),
            "customer_id": None,
            "amount_due": self._amount_from_money(money),
            "amount_paid": None,
            "status": invoice.get("status"),
            "created": invoice.get("detail", {}).get("invoice_date"),
            "currency": money.get("currency_code"),
            "hosted_invoice_url": invoice.get("href"),
            "invoice_pdf": None,
        }
    ...
    def _normalize_refund(self, refund: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "id": refund.get("id"),
            "amount": self._amount_from_money(amount),
            "currency": amount.get("currency_code"),
            "payment_intent_id": refund.get("capture_id"),
            "charge_id": refund.get("capture_id"),
