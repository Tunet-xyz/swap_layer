...
class SquarePaymentProvider(PaymentProviderAdapter):
    ...
    def __init__(
        ...
        self.base_url = (
            "https://connect.squareupsandbox.com" if sandbox else "https://connect.squareup.com"
        )
    ...
    def update_customer(
        ...
        if metadata:
            payload["reference_id"] = metadata.get("reference_id") or metadata.get("id")
        customer = self._request("PUT", f"/v2/customers/{customer_id}", json_body=payload).get(
            "customer", {}
        )
        return self._normalize_customer(customer)
    ...
    def create_product(
        ...
        **extra_params: Any,
        ...
        item = self._request("POST", "/v2/catalog/object", json_body=payload).get(
            "catalog_object", {}
        )
    ...
    def create_price(
        ...
        **extra_params: Any,
        ...
        variation = self._request("POST", "/v2/catalog/object", json_body=payload).get(
            "catalog_object", {}
        )
    ...
    def create_subscription(
        ...
        **extra_params: Any,
        ...
        plan_variation_id = (
            extra_params.pop("plan_variation_id", None)
            or price_id
            or (items or [{}])[0].get("price")
        )
        if not plan_variation_id:
            raise PaymentValidationError(
                "Square subscriptions require a plan_variation_id or price_id"
            )
        payload = {
            "idempotency_key": self._idempotency_key(idempotency_key),
        ...
        subscription = self._request("POST", "/v2/subscriptions", json_body=payload).get(
            "subscription", {}
        )
        return self._normalize_subscription(subscription)

    def get_subscription(self, subscription_id: str) -> dict[str, Any]:
        subscription = self._request("GET", f"/v2/subscriptions/{subscription_id}").get(
            "subscription", {}
        )
        return self._normalize_subscription(subscription)
    ...
    def update_subscription(
        ...
        **extra_params: Any,
        ...
        subscription = self._request(
            "PUT", f"/v2/subscriptions/{subscription_id}", json_body=payload
        ).get("subscription", {})
        return self._normalize_subscription(subscription)
    ...
    def cancel_subscription(
        ...
        subscription = self._request(
            "POST", f"/v2/subscriptions/{subscription_id}/cancel", json_body={}
        ).get("subscription", {})
        return (
            self._normalize_subscription(subscription)
            if subscription
            else {
                "id": subscription_id,
                "status": "canceled",
    ...
    def pause_subscription(
        self, subscription_id: str, behavior: str = "void", resumes_at: int | None = None
    ) -> dict[str, Any]:
        payload: dict[str, Any] = {}
        if resumes_at is not None:
            payload["pause_effective_date"] = resumes_at
        subscription = self._request(
            "POST", f"/v2/subscriptions/{subscription_id}/pause", json_body=payload
        ).get("subscription", {})
        return (
            self._normalize_subscription(subscription)
            if subscription
            else {"id": subscription_id, "status": "paused"}
        )
    ...
    def resume_subscription(
        ...
        payload: dict[str, Any] = {}
        subscription = self._request(
            "POST", f"/v2/subscriptions/{subscription_id}/resume", json_body=payload
        ).get("subscription", {})
        return (
            self._normalize_subscription(subscription)
            if subscription
            else {"id": subscription_id, "status": "active"}
        )
    ...
    def list_subscriptions(
        ...
        payload = self._request(
            "POST", "/v2/subscriptions/search", json_body={"query": query, "limit": limit}
        )
    ...
    def confirm_payment_intent(
        ...
        payment = self._request(
            "POST", f"/v2/payments/{payment_intent_id}/complete", json_body={}
        ).get("payment", {})
        return self._normalize_payment(payment)
    ...
    def create_checkout_session(
        ...
        **extra_params: Any,
        ...
        order_line_items = line_items or [
            {"name": extra_params.pop("name", price_id or "Item"), "quantity": str(quantity)}
        ]
        payload = {
            "idempotency_key": self._idempotency_key(idempotency_key),
        ...
        payload.update(extra_params)
        link = self._request("POST", "/v2/online-checkout/payment-links", json_body=payload).get(
            "payment_link", {}
        )
        return {
            "id": link.get("id"),
    ...
    def get_checkout_session(self, session_id: str) -> dict[str, Any]:
        link = self._request("GET", f"/v2/online-checkout/payment-links/{session_id}").get(
            "payment_link", {}
        )
        return {
            "id": link.get("id"),
    ...
    def create_invoice(self, customer_id: str, **kwargs: Any) -> dict[str, Any]:
        invoice = {
            ...
            "payment_requests": kwargs.pop(
                "payment_requests",
                [{"request_type": "BALANCE", "due_date": kwargs.pop("due_date", None)}],
            ),
        }
    ...
    def list_invoices(self, customer_id: str, limit: int = 10) -> list[dict[str, Any]]:
        payload = self._request(
            "GET", "/v2/invoices", params={"location_id": self.location_id, "limit": limit}
        )
        invoices = [self._normalize_invoice(invoice) for invoice in payload.get("invoices", [])]
        return [
            invoice for invoice in invoices if invoice.get("customer_id") in {None, customer_id}
        ]

    def finalize_invoice(self, invoice_id: str, **kwargs: Any) -> dict[str, Any]:
        invoice = self._request(
            "POST", f"/v2/invoices/{invoice_id}/publish", json_body=kwargs or {"version": 0}
        ).get("invoice", {})
        return self._normalize_invoice(invoice)
    ...
    def void_invoice(self, invoice_id: str, **kwargs: Any) -> dict[str, Any]:
        invoice = self._request(
            "POST", f"/v2/invoices/{invoice_id}/cancel", json_body=kwargs or {"version": 0}
        ).get("invoice", {})
        return (
            self._normalize_invoice(invoice) if invoice else {"id": invoice_id, "status": "voided"}
        )
    ...
    def create_refund(
        ...
        **kwargs: Any,
        ...
        if not payment_id:
            raise PaymentValidationError(
                "Square refunds require a payment_id via payment_intent_id or charge_id"
            )
        if amount is None:
    ...
    def list_refunds(
        self, payment_intent_id: str | None = None, charge_id: str | None = None, limit: int = 10
    ) -> list[dict[str, Any]]:
        ...
        payment_id = payment_intent_id or charge_id
        return (
            [refund for refund in refunds if refund.get("payment_intent_id") == payment_id]
            if payment_id
            else refunds
        )
    ...
    def verify_webhook_signature(
        ...
        if not signature_key or not notification_url:
            raise ValueError(
                "Square webhook_signature_key and webhook_notification_url are required"
            )
        raw_body = payload.decode("utf-8") if isinstance(payload, bytes) else payload
        signed_payload = f"{notification_url}{raw_body}".encode()
    ...
    def _normalize_customer(self, customer: dict[str, Any]) -> dict[str, Any]:
        name = (
            " ".join(
                part for part in [customer.get("given_name"), customer.get("family_name")] if part
            )
            or None
        )
        return {
            ...
            "metadata": {"reference_id": customer.get("reference_id")}
    ...
    def _normalize_product(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "active": not item.get("is_deleted", False),
            "metadata": {},
        }
    ...
    def _normalize_price(self, variation: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "amount": self._amount_from_money(money),
            "currency": money.get("currency"),
            "recurring": None,
            "active": not variation.get("is_deleted", False),
            "metadata": {},
        }
    ...
    def _normalize_subscription(self, subscription: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "current_period_end": subscription.get("charged_through_date"),
            "cancel_at_period_end": False,
            "canceled_at": subscription.get("canceled_date"),
            "items": [{"price_id": subscription.get("plan_variation_id"), "quantity": 1}],
            "metadata": {},
        }
    ...
    def _normalize_payment(self, payment: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "client_secret": None,
            "metadata": {"receipt_url": payment.get("receipt_url")}
    ...
    def _normalize_invoice(self, invoice: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "amount_due": self._amount_from_money(total),
            "amount_paid": None,
            "status": (invoice.get("status") or "").lower(),
            "created": invoice.get("created_at"),
            "currency": total.get("currency"),
            "hosted_invoice_url": payment_requests[0]
    ...
    def _normalize_refund(self, refund: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "amount": self._amount_from_money(money),
            "currency": money.get("currency"),
            "payment_intent_id": refund.get("payment_id"),
            "charge_id": refund.get("payment_id"),
